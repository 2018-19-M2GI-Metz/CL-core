...
/*static*/ PElement< std::pair<Sommet<InfoSommet>*, double> > * OutilsCarte::listeVoisins(const Sommet<InfoSommet> * s, const Graphe<InfoArete, InfoSommet> & graphe)
{
	PElement< std::pair < Sommet<InfoSommet> *, Arete<InfoArete, InfoSommet> * > > * ladj, *lA;						// pair < Sommet<InfoSommet> *, Arete<InfoArete,InfoSommet> * >

	ladj = graphe.adjacences(s);

	PElement< std::pair<Sommet<InfoSommet>*, double> > * lVC;

	for (lA = ladj, lVC = NULL; lA; lA = lA->s)
		lVC = new PElement< std::pair<Sommet<InfoSommet>*, double> >(new std::pair<Sommet<InfoSommet>*, double>(lA->v->first, lA->v->second->v.cout1), lVC);


	PElement< std::pair< Sommet<InfoSommet> *, Arete<InfoArete, InfoSommet> * > >::efface2(ladj);
	return lVC;
