...
template <class S, class T>
std::ostream & operator << (std::ostream & os, const Graphe<S, T> & gr) { return os << (std::string)gr; }
...
PElement< std::pair< Sommet<T> *, Arete<S, T>* > >  *  Graphe<S, T>::adjacences(const Sommet<T> * sommet) const
{
	...
	PElement< std::pair< Sommet<T> *, Arete<S, T>* > > * r;				// pair< Sommet<T> *, Arete<S,T>* >

	for (l = lAretes, r = NULL; l; l = l->s)

		if (sommet == l->v->debut)
			r = new PElement< std::pair< Sommet<T> *, Arete<S, T>* > >(new std::pair< Sommet<T> *, Arete<S, T>* >(l->v->fin, l->v), r);
		else
			if (sommet == l->v->fin)
				r = new PElement< std::pair< Sommet<T> *, Arete<S, T>* > >(new std::pair< Sommet<T> *, Arete<S, T>* >(l->v->debut, l->v), r);
...
Graphe<S, T>::operator std::string() const
{
	...
	oss << "Graphe( \n";
	oss << "prochaine clef = " << this->prochaineClef << '\n';
...
bool Graphe<S, T>::dessineToutesAretes(FENETRE & fenetre) const
{
	...
	for (pA = this->lAretes; pA; pA = pA->s)
		if (!fenetre.dessine(pA->v)) 
			return false; // tente de dessiner puis retourne false en cas d'échec
